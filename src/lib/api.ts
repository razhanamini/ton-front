...
class ApiClient {
    ...
    private getTelegramHeaders(): Record<string, string> {
        ...
        // If we're in Telegram WebApp, use real auth
        if (tg && tg.initData) {
            console.log('Using Telegram auth'); // Debug log
            return {
        ...
        if (import.meta.env.DEV) {
            console.warn('Dev mode: using mock auth');
            return {
                'Content-Type': 'application/json',
                'X-Dev-Mode': 'true'
            };
        ...
        // Production without Telegram - this shouldn't happen
        console.error('Not running in Telegram WebApp and not in dev mode');
        return {
            'Content-Type': 'application/json'
        };
    }
